...
import { createShellV2 } from '../ui/v2/shell.js';
import { initializeAppearanceClock } from '../ui/appearance.js';
import { renderHomeV2 } from '../ui/v2/home.js';
import { renderMovementsV2 } from '../ui/v2/movements.js';
import { renderPlanning } from '../ui/screens/planning.js';
import { renderAccounts } from '../ui/screens/accounts.js';
import { openAccountsManager } from '../ui/v2/accounts-manager.js';
import { bindStableAppPreview } from '../ui/v2/app-preview.js';
...
async function start() {
    ...
    const shell = createShellV2(profile, (next) => { route = next; void render(); }, () => {
    ...
    async function render() {
        ...
        try {
            let screen;
            if (route === 'home')
                screen = await renderHomeV2(repositories, profile, {
                    onOpenAccounts: () => openAccountsManager(repositories, profile, entityLifecycle, () => void render()),
                    onOpenPlanning: () => { route = 'planning'; void render(); }
                });
            else if (route === 'movements')
                screen = await renderMovementsV2(repositories, profile, transactionMutations, () => void render());
