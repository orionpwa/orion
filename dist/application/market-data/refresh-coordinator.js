import { refreshPortfolioQuotes } from './refresh-portfolio.js';
function keyFor(profileId) {
    return `profile:${profileId}:market:last_refresh_at`;
}
export class MarketRefreshCoordinator {
    instruments;
    gateway;
    metadata;
    runtime;
    ...
    async refreshIfDue(force = false) {
        ...
        this.running = refreshPortfolioQuotes(this.instruments, this.gateway, this.profileId, now)
            .then(async (result) => {
